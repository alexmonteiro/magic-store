# encoding: UTF-8
# WARNING
# Uncomment it to DELETE ALL DATA
=begin
...
# Option Types (New, Foil)
op = Spree::OptionType.create(:name => "Qualidade", :presentation => "Qualidade")
Spree::OptionValue.where(:name => "novo", :presentation => "Novo", :option_type_id => op.id).first_or_create
Spree::OptionValue.where(:name => "foil", :presentation => "Foil", :option_type_id => op.id).first_or_create
options_values = Spree::OptionValue.where(:option_type_id => op.id)
...
if p = Spree::Product.create(:price => 0.0, :name => "Ajani, Convocador do Bando",	:description => "<p>Planeswalker — Ajani (Loyalty: 4), 1WW (3)</p><p class=\"ctext\"><b>+1: Coloque um marcador +1/+1 sobre até uma criatura alvo.<br><br>-3: A criatura alvo ganha voar e golpe duplo até o final do turno.<br><br>-8: Coloque X fichas de criatura brancas 2/2 do tipo Felino no campo de batalha, sendo X igual ao seu total de pontos de vida.</b></p><p><i></i></p><p>Ilus. D. Alexander Gregory</p>")
  ...
  v.option_values = options_values
  v.save!
  Spree::Image.create(:attachment => File.open("#{path}#{i}.jpg"), :viewable_id => p.master.id, :alt => "#{p.name}", :viewable_type => 'Spree::Variant')
  Spree::ProductOptionType.where(:product_id => p.id, :option_type_id => op.id).first_or_create
end
begin
